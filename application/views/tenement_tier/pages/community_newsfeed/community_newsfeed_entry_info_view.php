...
<div class="page-container">
    <div class="tab-pane active" id="add-entry">
        ...
        <div class="controls">
            <?php
                list($date, $time) = split(' ', $entry[0]['tmtnews_date']);
                list($y, $m, $d) = split('[:/.-]', $date);
            ?>
            <label for="input" class="control-label">News/Event Date: <i>(optional)</i></label>
            <input id="entry-date" name="entry-date" type="text" value="<?php echo "$m/$d/$y"; ?>" class="datepicker">
        </div>
        <div class="controls">
        ...
        <div class="controls">
            ...
            <div class="controls">
                <input name="entry-title" id="entry-title" class="input-xxlarge" value="<?= $entry[0]['tmtnews_title']; ?>" type="text" required>
        ...
        <div class="controls">
            ...
            <div class="controls">
                <textarea class="input-xxlarge" id="entry-description"><?= $entry[0]['tmtnews_details']; ?></textarea>

            </div>
        </div>
        <br />
        <span onclick="updateNewsFeedEntry(<?= $entry[0]['tmtnews_id']; ?>);" class="btn btn-wuxia btn-large btn-primary" type="submit">Update Entry</span>
...
(function() {
    ...
    <?php if($entry[0]['tmtnews_stime'] != NULL): ?>
        $('#stime-selected').val('yes');
        $('#stime-selection').show();
        $('#entry-stime-hour').val(<?= $entry[0]['stime_hour']; ?>);
        $('#entry-stime-minute').val(<?= $entry[0]['stime_minute']; ?>);
        $('#entry-stime-ampm').val('<?= $entry[0]['stime_ampm']; ?>');
        $('#stime-selection-checkbox').attr('checked', true);
    <?php endif; ?>
    
    <?php if($entry[0]['tmtnews_etime'] != NULL): ?>
        $('#etime-selected').val('yes');
        $('#etime-selection').show();
        $('#entry-etime-hour').val(<?= $entry[0]['etime_hour']; ?>);
        $('#entry-etime-minute').val(<?= $entry[0]['etime_minute']; ?>);
        $('#entry-etime-ampm').val('<?= $entry[0]['etime_ampm']; ?>');
        $('#etime-selection-checkbox').attr('checked', true);
    <?php endif; ?>
